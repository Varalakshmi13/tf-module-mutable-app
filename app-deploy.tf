resource "null_resource" "app-deploy" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ansible-pull -U https://github.com/varalakshmi13/ansible roboshop.yml  -e role_name=${var.COMPONENT} -e HOST=localhost -e APP_VERSION=${var.APP_VERSION} -e ENV=${var.ENV} -e MONGODB_ENDPOINT=${data.terraform_remote_state.db.outputs.MONGODB_ENDPOINT}"
    ]
  }
}